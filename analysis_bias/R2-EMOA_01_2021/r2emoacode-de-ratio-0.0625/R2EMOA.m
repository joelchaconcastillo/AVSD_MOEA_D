...
%
% [5] Carlos M. Fonseca, Lu�s Paquete, and Manuel L�pez-Ib��ez. An improved
...
% options: general - these are evaluated once [PARETOFRONT, PARETOSET] = R2EMOA('WFG8', struct('nObj', '2', 'nVar','24', 'seed','1', 'f_name','OOOOOOO'))
...
defopts.maxEval           = '2500000           % maximum number  f evaluations';
defopts.useOCD            = 'false          % use OCD to detect convergence';
...
defopts.var_crossover_dist= '2            % distribution index for crossover';
defopts.var_mutation_prob = '1./nVar       % variable mutation probability';
defopts.var_mutation_dist = '50            % distribution index for mutation';
...
nObj= myeval(opts.nObj);
nVar= myeval(opts.nVar);
seed= myeval(opts.seed);
f_name=opts.f_name;
...
[rngMin rngMax isInt problemCall] = feval(initProblemStr, nVar);
...
active = setdiff(1:nPop+1,elementsOffspring);
PS = population(active,1:nVar);
PF = population(active,nVar+1:nVar+nObj);
save(f_name, 'PF', '-ascii', '-append' );
save(strcat(f_name,'VAR'), 'PS', '-ascii' ,'-append');
r1=countEval;
r2=countEval;
totalcountr=0;
while ~terminationCriterion && (countEval < maxEval)    
    ...
    r2=countEval;
%    if mod(countEval,floor(0.1.*maxEval)) == 0
    totalcountr = totalcountr + (r2-r1);
    if totalcountr > floor(0.1.*maxEval)
	active = setdiff(1:nPop+1,elementsOffspring);
        totalcountr = totalcountr - floor(0.1.*maxEval);
	PS = population(active,1:nVar);
	PF = population(active,nVar+1:nVar+nObj);
        save(f_name, 'PF', '-ascii', '-append' );
        save(strcat(f_name,'VAR'), 'PS', '-ascii' ,'-append');
...
%if mod(countEval, 250) == 0
%    save(f_name, 'paretoFront', '-ascii', '-append' );
%    save(strcat(f_name,'VAR'), 'paretoSet', '-ascii' ,'-append');
%end;
if nargout == 3
    if ~terminationCriterion
...
PF = population(active,nVar+1:nVar+nObj);
save(f_name, 'PF', '-ascii', '-append' );
%save(f_name, 'countEval', '-ascii', '-append' );
save(strcat(f_name,'VAR'), 'PS', '-ascii' ,'-append');
...
for i = 1 : nPop
    ...
    f(i,nVar+1:nVar+nObj) = feval(problemCall, [f(i,1:nVar),nObj]);
end;
...
for o=1:nOffspring
    if useDE
		...
		if offspring(j) < rngMin(j)
		   %offspring(j) = rngMin(j) + rand(1)*(parent(1,j) - rngMin(j));
		   offspring(j) = parent(1,j);
		end;
		if offspring(j) > rngMax(j)
		   %offspring(j) = rngMax(j) - rand(1)*(rngMax(j) - parent(1,j));
		   offspring(j) = parent(1,j);
		end;


%		if offspring(j) > rngMax(j) || offspring(j) < rngMin(j)
%                    offspring(j)=parent(1,j);
%		end;
            end;
        end;
        for j = 1 : nVar
            if rand(1) < variable_mutation_prob
  	       y  = offspring(j);
               ...
               delta2 = (yu-y)/(yu-yl);
               rnd = rand(1);
               mut_pow = 1.0/(variable_mutation_dist+1.0);
               if rnd <= 0.5
                   xy = 1.0-delta1;
                   val = 2.0*rnd+(1.0-2.0*rnd)*(xy^(variable_mutation_dist+1.0));
                   deltaq =  val^(mut_pow) - 1.0;
               else
                   xy = 1.0-delta2;
                   val = 2.0*(1.0-rnd)+2.0*(rnd-0.5)*(xy^(variable_mutation_dist+1.0));
                   deltaq = 1.0 - (val^(mut_pow));
               end
                offspring(j) = offspring(j) + deltaq*(yu-yl);
            end
        end
    else
        % use SBX & PM
	...
	%This just indicates the old SBX crossover...
        %for j = 1 : nVar
        ...
        %        child_2(j) = swap;
        %    end;
	%%This is the official nsga-ii version of the SBX, which considers the variable bounds
        EPS = 1e-14;
        if rand(1) < variable_crossover_prob 
	   for j = 1 : nVar
	    if rand(1) <= 0.5
	      if abs(parent_1(j)-parent_2(j))>EPS
	       ...
	       yu = rngMax(j);
               ...
               alpha = 2.0 - beta^(-(variable_crossover_dist+1.0));
	       if rand(1) <= (1.0/alpha)
                  betaq = (rand(1)*alpha)^(1.0/(variable_crossover_dist+1.0));
               else
                  betaq = (1.0/(2.0 - rand(1)*alpha))^(1.0/(variable_crossover_dist+1.0));
	       end;
    ...
    offspring(nVar + 1: nVar + nObj) = feval(problemCall, [offspring(1:nVar), nObj]);
