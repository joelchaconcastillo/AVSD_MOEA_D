...
void CMOEAD::evol_population()
{
    ...
    for(int sub=0; sub<order.size(); sub++)
	{
		...
		CIndividual child;
		double rate2 = 0.5;//box_muller(0.5,0.1); //rate + 0.25*(rnd_uni(&rnd_uni_init) - 0.5);
//		diff_evo_xoverB(population[c_sub].indiv,population[plist[0]].indiv,population[plist[1]].indiv, child, rate2);
		diff_evo_xoverA(population[c_sub].indiv,population[plist[0]].indiv,population[plist[1]].indiv, population[plist[2]].indiv, child, CR, F);
		plist.clear();
...
void CMOEAD::exec_emo(int run)
{
	...
	// initialization
    nfes      = 0;
    sprintf(filename1,"%s/POS/POS_MOEAD_%s_RUN%d_seed_%d_nobj_%d_niche_%d.dat_bounded_CR_%lf_F_%lf_%s",strpath, strTestInstance,run, seed, nobj, niche, CR, F, description);
    sprintf(filename2,"%s/POF/POF_MOEAD_%s_RUN%d_seed_%d_nobj_%d_niche_%d.dat_bounded_CR_%lf_F_%lf_%s",strpath, strTestInstance,run, seed, nobj, niche, CR, F, description);
