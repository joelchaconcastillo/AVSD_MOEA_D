...
// Los primeros dos argumentos es el nombre del programa + Nombre del algoritmo + Instancia + Numero de variables + Numero de objetivos + Semilla
   if(argc == 10)
   {
        ratiobias = atof(argv[5]);
	argc--;
        Seed = atoi(argv[6]);
        argc--;
        DirReport = string(argv[7]);
        argc--;
        CR = atof(argv[8]);
        argc--;
        F =atof(argv[9]);
...
//	indicators = NULL ;
  ...
  string FileNameObjective = DirReport + "/OBJ_NSGAII_SBX_POLYNOMIAL_EVALUATIONS_" +to_string(maxEvaluations) +"_"+problem->getName() +"_SEED_"+to_string(Seed)+"_"+to_string(problem->getNumberOfObjectives())+"_CR_"+to_string(CR)+"_F_"+to_string(F)+"_b_"+to_string(ratiobias)+".txt";
  string FileNameVariable = DirReport + "/VAR_NSGAII_SBX_POLYNOMIAL_EVALUATIONS_" +to_string(maxEvaluations) +"_"+problem->getName() +"_SEED_"+to_string(Seed)+"_"+to_string(problem->getNumberOfObjectives())+"_CR_"+to_string(CR)+"_F_"+to_string(F)+"_b_"+to_string(ratiobias)+".txt";
