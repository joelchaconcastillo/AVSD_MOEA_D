...
const double BT7::PI = 3.141592653589793;

BT7::BT7(string solutionType, int numberOfVariables, int numberOfObjectives, double ratiobias) {
...
long double BT7::D1(long double g, long double theta)
{
 ...
 */
void BT7::evaluate(Solution *solution) {
	XReal * vars = new XReal(solution);
   long double sum1 = 0.0, sum2 = 0.0, theta = 1.0e-3*ratiobias_;
   for(int j = 2; j <= numberOfVariables_; j++)
   {
      long double yj = vars->getValue(j-1) - sin(6.0*M_PI* vars->getValue(0));
