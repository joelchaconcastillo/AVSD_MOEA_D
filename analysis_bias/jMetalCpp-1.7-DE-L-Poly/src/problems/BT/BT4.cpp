...
const double BT4::PI = 3.141592653589793;

BT4::BT4(string solutionType, int numberOfVariables, int numberOfObjectives, double ratiobias) {
...
long double BT4::D1(long double g, long double theta)
{
   return (g*g) + (1.0-exp(-(g*g)/theta))/5.0;
}
long double BT4::S2(long double x,long double gamma)
{
 ...
 */
void BT4::evaluate(Solution *solution) {
	XReal * vars = new XReal(solution);

 long double sum1 = 0.0, sum2 = 0.0, theta = 1.0e-8*ratiobias_;
   ...
   solution->setObjective(0, S2(vars->getValue(0), 0.06) + sum1);
   solution->setObjective(1, 1.0 - sqrt(S2(vars->getValue(0), 0.06)) + sum2);
