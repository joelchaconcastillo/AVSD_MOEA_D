...
const double BT6::PI = 3.141592653589793;

BT6::BT6(string solutionType, int numberOfVariables, int numberOfObjectives, double ratiobias) {
...
long double BT6::D1(long double g, long double theta)
{
 ...
 */
void BT6::evaluate(Solution *solution) {
	XReal * vars = new XReal(solution);

   long double sum1 = 0.0, sum2 = 0.0, theta = 1.0e-4*ratiobias_;
   for(int j = 2; j <= numberOfVariables_; j++)
   {
      long double yj = vars->getValue(j-1) - pow(vars->getValue(0),0.5+((1.5*(j-1.0))/(numberOfVariables_-1.0)));
      if(!(j%2)) sum1 += D1(yj, theta);
      if(j%2) sum2 += D1(yj, theta);
  }
   solution->setObjective(0, vars->getValue(0) + sum1 );
   solution->setObjective(1, 1.0 - sqrt(vars->getValue(0)) + sum2 );
   delete vars ;
} // evaluate
