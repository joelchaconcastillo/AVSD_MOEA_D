...
float DifferentialEvolutionCrossover::box_muller(float m, float s) {
   float x1, x2, w, y1;
        static float y2;
        ...
        {
                do {
                        x1 = 2.0 * PseudoRandom::randDouble(0, 1) - 1.0;
                        x2 = 2.0 * PseudoRandom::randDouble(0, 1) - 1.0;
                        w = x1 * x1 + x2 * x2;
                } while ( w >= 1.0 );
...
void * DifferentialEvolutionCrossover::execute(void *object) {
  ...
  jrand = PseudoRandom::randInt(0, numberOfVariables - 1);
   ...
   // CR_ = ((PseudoRandom::randDouble(0, 1)<0.5)?0.2:0.9 );
   // F_ = box_muller(0.5,0.1);
